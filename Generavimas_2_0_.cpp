...
//  Skaito student? duomenis i? failo ir saugo ? list<Studentas>.
...
void Generavimas(int n) {
    ...
    if (!out) {//ar atsidaro gerai 
        cerr << "Klaida atidarant fail? rasymui: " << filename << endl;
        return;
    }
...
void RusiuotiV(std::vector<Studentas>& studentai, std::vector<Studentas>& vargsiukai, std::vector<Studentas>& kietiakiai, char choice2) {
    ...
    std::cout << "Rusiavimas u?truko: " << elapsedSorting.count() << " sekundes" << std::endl;
}
